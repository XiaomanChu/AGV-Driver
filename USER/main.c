  ...
  TIM3_Cap_Init(0xffff,84-1); //ÒÔ1MhzµÄÆµÂÊ¼Æ
   ...
   while(1) 
	...
	//printf("HIGH:%lld us\r\n",count1); 
		...
		int flag=0;			//Êµ¼ÊÎ»ÖÃÓëÉè¶¨Î»ÖÃÒ»ÖÂ
		if(pst==pst_old)//ÅĞ¶Ïµ±Ç°Î»ÖÃÓëÉè¶¨Î»ÖÃ
		{
			delay_ms(100);
			flag=1;
		}
		else flag=0;
		if(pst>15&&pst<30&&pst!=pst_old)//×ó×ª
		{
			//printf("PST:%lld us\r\n",(long long)pst);
		...
		else if(pst>0&&pst<15&&pst!=pst_old)//ÓÒ×ª
		{
			//printf("PST:%lld us\r\n",(long long)pst);
			Locate_Abs(-45000+(int)k3*(pst-8),7000);
			pst_old=pst;
			//printf("PST_OLD:%lld us\r\n",(long long)pst_old);
		}
